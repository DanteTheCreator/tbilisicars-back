...
def list_locations(db: Session = Depends(get_db), skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=1000)):
    items = db.query(Location).offset(skip).limit(limit).all()
...
def create_location(payload: Dict[str, Any], db: Session = Depends(get_db)):
    obj = Location()
...
def update_location(item_id: int, payload: Dict[str, Any], db: Session = Depends(get_db)):
    obj = db.get(Location, item_id)
    if not obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Location not found")
    apply_updates(obj, payload)
